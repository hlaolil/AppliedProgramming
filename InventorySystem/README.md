# Overview

Here I showcase my C# programming skills through an inventory management software that manages stock levels and expiration. 

It is a console application that tracks stock items, their quantity, price, shelf location and reorder level. I saves them to a file so the data survives between runs.

The purpose of writing this programme was to demonstrate object-oriented programming in C#. The project is a vehicle for understanding classes, abstract classes, inheritance, and the difference between a class and a struct.

Here is a link to the demonstration video that walks through the code:

[Software Demo Video](https://youtu.be/9yk7HAJ8_zM)

# Development Environment

I developed this program on Windows using Visual Studio Code as my editor, with the C# Dev Kit extension.

# Useful Websites

Here is a list of websites that I found helpful in this project

- [Learning Content](https://byui-cse.github.io/cse310-ww-course/week01/activity_portfolio.html)

# Future Work

Here is a list of things that I need to fix, improve, and add in the future.

- Move from console to a graphical interface

